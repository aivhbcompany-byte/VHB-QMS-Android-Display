(() => {
  ...
  function pageLooksLikeCounterDisplay(bodyText) {
    ...
    return folded.includes("QUAY") && (
      folded.includes("DANG PHUC VU") ||
      folded.includes("DANG GOI") ||
      folded.includes("SAN SANG PHUC VU")
    );
  }
  ...
  function readyState(bodyText) {
    const folded = asciiFold(bodyText);
    return folded.includes("SAN SANG PHUC VU")
      || folded.includes("SAN SANG")
      || folded.includes("CHO GOI SO");
  }
  ...
  function detectState() {
    ...
    if (readyState(bodyText) && !activeState(bodyText)) return null;
  ...
  function sendMessage(payload) {
    try { browser.runtime.sendNativeMessage(NATIVE_APP, payload); } catch (_) { }
  }
