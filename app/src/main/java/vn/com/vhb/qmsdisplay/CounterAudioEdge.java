...
final class CounterAudioEdge {
    ...
    private void handleState(String ticket, String counter, String callId, boolean initial) {
        ...
        // Prime only: never replay the number already visible after reboot/reload/reconnect.
        if (initial) {
            lastObservedKey = key;
            return;
        }
        ...
        // Observe while disabled so enabling later does not replay a stale number.
        ...
        if (!tokens.isEmpty()) enqueue(key, tokens, repeats, gapMs, volume, false);
    }
    ...
    private void appendCodeTokens(List<String> out, String value) {
        ...
        for (int i = 0; i < upper.length(); i++) {
            ...
            // Current numeric pack does not need A-Z. Add a letter only when its WAV exists.
            if (c >= 'A' && c <= 'Z' && !hasFile(token)) continue;
            out.add(token);
        }
    }
    ...
    private static boolean validateWav(File file) {
        ...
        try (RandomAccessFile in = new RandomAccessFile(file, "r")) {
            ...
            while (in.getFilePointer() + 8L <= in.length()) {
                ...
                if (start + size > in.length()) return false;
